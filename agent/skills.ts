...
const SETTLEMENT_INTENT = /要不要|是否|需不需要|需要吗|怎么|如何|怎样|什么时候|何时|是什么|指什么|什么(?:规则|要求|格式|内容|材料)|规则(?:是什么|怎么|如何|怎样|呢|吗)|解释|说明(?!函)|命名规则|上传(?:规则|流程|要求|方式)|可以吗|能否|必须.{0,12}吗|是不是必须/;
