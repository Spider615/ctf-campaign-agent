...
容器部署（butler）用根目录 `Dockerfile`，单容器跑页面（wrangler 本地模式 + 本地 D1，数据在 `D1_PERSIST_DIR`，默认 `/data/d1`）和 Agent 服务，入口 `scripts/start-container.mjs` 负责迁移、进程联动，并把 Agent 固定在容器内回环地址，只需配 `DEEPSEEK_API_KEY`（契约见 `.env.example`）。分开部署时 Agent 服务单独跑在有 Node.js 和可写磁盘的环境；只有这时 Workers 才配 `AGENT_SERVICE_URL`，两边配同一个 `AGENT_SERVICE_TOKEN`。Agent 部署产物必须包含：
...
- `card.ts`：结构化回答（面板修改、提议）的解析和校验。`messages.ts`：消息结构 `StoredMessage`（v2，`agent_round_card` / `agent_readback` / `user_card_submit` 只为读旧会话）、`flowOf`、改动摘要。
...
- `ask_campaign_questions` 登记这句回复要问的题号和提议：问题只收当前缺项（不缺的略过），一次最多 3 件，提议走 `checkProposal`；用户这句说了让扣点「不知道」时不能提议没有。`accept_campaign_proposals` 只在上一句有提议、quote 在原话里、是明确同意且前面不是「不 / 没」时生效；整句不是纯点头时必须列出同意的题号。
