...
import type { AgentTraceEvent } from "../../lib/tool-trace";
...
export function ToolStep({ event, live, showSupplement = false }: { event: AgentTraceEvent; live: boolean; showSupplement?: boolean }) {
  ...
  return (
    <li className="grid grid-cols-[24px_minmax(0,1fr)_auto] gap-x-2.5 rounded-xl border border-[#e1ecf8] bg-white/80 px-3 py-2.5 shadow-[0_3px_12px_rgba(43,94,151,0.04)]">
      <span className={`mt-0.5 grid size-6 place-items-center rounded-full ${iconSurface} ${tone}`}>
      ...
      <div className="min-w-0">
        <div className="flex min-w-0 flex-wrap items-center gap-1.5">
          ...
          {showSupplement ? <span className="rounded-full bg-[#fff4dc] px-1.5 py-0.5 text-[10px] font-medium text-[#9a620d]">系统补跑</span> : null}
          {event.initiatedBy === "orchestrator" && !showSupplement ? <span className="rounded-full bg-[#eef5ff] px-1.5 py-0.5 text-[10px] font-medium text-[#4f72a1]">确定性执行</span> : null}
        </div>
      ...
      <span className={`mt-0.5 inline-flex items-center gap-1 whitespace-nowrap text-[11px] font-medium ${tone}`}>
        {event.status === "started" ? <CircleDot className="size-3" aria-hidden="true" /> : null}
        {event.durationMs === undefined ? STATUS_COPY[event.status] : `${(event.durationMs / 1000).toFixed(1)}s`}
      </span>
    </li>
  );
}
