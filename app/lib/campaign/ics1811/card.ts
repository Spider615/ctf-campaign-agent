// 结构化回答 → 事实层。面板里手改、用户同意 Agent 的提议都走这里：值是用户亲手选的或点头同意的，
// 记为用户来源，不做原话核对；每个值仍按代码表和取值范围校验。

import { categoryByCode, storeByCode } from "./codebook.ts";
import { emptyItem, mergeOffer, setFact } from "./facts.ts";
import { isRealDate } from "./phrases.ts";
import type { FactVia, Ics1811Draft, OfferItem, OfferPattern, QuestionId } from "./types.ts";

export type CardResult = { draft: Ics1811Draft; applied: QuestionId[]; ignored: Array<{ id: string; reason: string }> };

type Answer = Record<string, unknown>;
type Handler = (draft: Ics1811Draft, answer: Answer, quote: string, via: FactVia) => string | null;
...
const offerHandler = (requirePattern: boolean): Handler => (draft, answer, quote, via) => {
  ...
  setFact(draft, "offer", mergeOffer(existing, { pattern, items: items as OfferItem[], unsupportedType: null }), quote, via);
  return null;
};

const HANDLERS: Record<QuestionId, Handler> = {
  Q1: (draft, answer, quote, via) => {
    const { start, end } = answer;
    if (typeof start !== "string" || typeof end !== "string" || !DATE.test(start) || !DATE.test(end)) return "日期要填 YYYY-MM-DD";
    if (!isRealDate(start) || !isRealDate(end)) return "日期不存在";
    if (start > end) return "开始日期不能晚于结束日期";
    setFact(draft, "dates", { start, end }, quote, via);
    return null;
  },
  Q2: (draft, answer, quote, via) => {
    const stores = codes(answer.stores);
    if (!stores.length || stores.some((code) => !storeByCode(code))) return "请从门店列表里选择";
    setFact(draft, "stores", stores, quote, via);
    draft.unresolvedStores = [];
    return null;
  },
  Q3: offerHandler(true),
  Q3a: offerHandler(false),
  Q3b: (draft, answer, quote, via) => {
    if (typeof answer.editable !== "boolean") return "请选择门店能不能改价";
    setFact(draft, "discountEditable", answer.editable, quote, via);
    return null;
  },
  Q3c: (draft, answer, quote, via) => {
    if (answer.repeat !== "once" && answer.repeat !== "every") return "请选择减一次还是每满都减";
    setFact(draft, "thresholdRepeat", answer.repeat, quote, via);
    return null;
  },
  Q3d: (draft, answer, quote, via) => {
    if (answer.basis !== "actual" && answer.basis !== "whole") return "请选择按实际克重还是按整克";
    setFact(draft, "gramBasis", answer.basis, quote, via);
    return null;
  },
  Q3e: (draft, answer, quote, via) => {
    ...
    setFact(draft, "offer", { ...offer.value, items }, quote, via);
    return null;
  },
  Q4: (draft, answer, quote, via) => {
    ...
    setFact(draft, "categories", { ...(draft.facts.categories?.value ?? {}), ...slots }, quote, via);
    draft.unresolvedCategories = [];
    return null;
  },
  Q4a: (draft, answer, quote, via) => {
    if (typeof answer.convert !== "boolean") return "请选择要不要转 outlet 餐牌";
    setFact(draft, "menuConversion", answer.convert, quote, via);
    return null;
  },
  Q5a: (draft, answer, quote, via) => {
    if (answer.none === true) {
      setFact(draft, "rates", { concession: 0, collection: 0 }, quote, via);
      return null;
    }
    ...
    setFact(draft, "rates", { concession, collection }, quote, via);
    return null;
  },
  Q5b: (draft, answer, quote, via) => {
    if (answer.commission !== "actual_price" && answer.commission !== "price_times_discount") return "请选择提成口径";
    setFact(draft, "commission", answer.commission, quote, via);
    return null;
  },
  Q5c: (draft, answer, quote, via) => {
    if (typeof answer.has !== "boolean") return "请选择有没有结算说明函";
    setFact(draft, "settlementLetter", answer.has, quote, via);
    return null;
  },
  Q6a: (draft, answer, quote, via) => {
    if (answer.wanted === false) {
      setFact(draft, "slogan", { wanted: false }, quote, via);
      return null;
    }
    ...
    setFact(draft, "slogan", { wanted: true, text, legalConfirmed: answer.legalConfirmed }, quote, via);
    return null;
  },
  Q6b: (draft, answer, quote, via) => {
    const slogan = draft.facts.slogan?.value;
    if (!slogan?.wanted || typeof answer.legalConfirmed !== "boolean") return "请选择标语法务是否确认过";
    setFact(draft, "slogan", { ...slogan, legalConfirmed: answer.legalConfirmed }, quote, via);
    return null;
  },
};

// source 不给时按面板和旧卡片记（quote「卡片：Q1」）；用户同意提议时传入他那句话。
export function applyCardAnswers(input: Ics1811Draft, answers: unknown, source?: { quote: string; via: FactVia }): CardResult {
  const draft = structuredClone(input);
  const applied: QuestionId[] = [];
  const ignored: CardResult["ignored"] = [];
  if (!isRecord(answers)) return { draft, applied, ignored };
  for (const key of Object.keys(answers)) {
    if (!ORDER.includes(key as QuestionId)) ignored.push({ id: key, reason: "不认识的问题" });
  }
  for (const id of ORDER) {
    if (!(id in answers)) continue;
    const raw = answers[id];
    const error = isRecord(raw) ? HANDLERS[id](draft, raw, source?.quote ?? `卡片：${id}`, source?.via ?? "card") : "回答内容不完整";
