...
- 系统提示词要求工具调用前不输出用户可见铺垫；若候选 message 后续出现 `tool_use`，立即丢弃该候选并发送 `text_reset`；
- 候选文本按完整句或完整 Markdown 行经过与 `finishAgentTurn` 同源的安全过滤后才发送，不能短暂显示最终会被删除的问句、预估或超长内容；
